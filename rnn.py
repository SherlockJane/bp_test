...
class RecurrentLayer(object):
    def __init__(self, input_width, state_width,
        ...
        self.U = np.random.uniform(-1e-4, 1e-4, (state_width, input_width))  # 权重数组
        self.W = np.random.uniform(-1e-4, 1e-4, (state_width, state_width))  # 权重数组
    ...
    def backward(self, sensitivity_array, activator):  # BPTT算法
        self.calc_delta(sensitivity_array, activator)
        self.calc_gradient()

    def calc_delta(self, sensitivity_array, activator):
        self.delta_list = []  # 各个时刻的误差项
    ...
    def calc_gradient(self):
        ...
        self.gradient = func.reduce(lambda a, b: a + b, self.gradient_list, self.gradient_list[0])

    def calc_gradient_t(self, t):
        gradient = np.dot(self.delta_list[t], self.state_list[t - 1].T)
        self.gradient_list[t] = gradient

    def update(self):  # 梯度下降法更新权重
